...
from app.db.models import APIKey, MealLog, ScaleWeight, UserProfile
...
def import_data(username="victor"):
    ...
    try:
        ...
        if not user:
            user = UserProfile(username=username, dob="1987-12-07", height_cm=185.0, sex="male")
            db.add(user)
            db.commit()
        ...
        if not key_obj:
            key_obj = APIKey(key="ctk_live_victor_dev_key", username=username, name="Victor Personal Key")
            db.add(key_obj)
            db.commit()

        export_dir = os.getenv("DATA_EXPORT_DIR", "data_export")

        if not os.path.exists(export_dir):
            print(f"Directory '{export_dir}' does not exist. Skipping historical data import.")
            return

        # 1. Import Scale Weights
        weight_files = glob.glob(os.path.join(export_dir, "*.xlsx"))
        weight_count = 0
                ...
                for r in rows[1:]:
                    if len(r) >= 2 and r[0] and r[1]:
                        ...
                        existing_w = (
                            ...
                            .filter(ScaleWeight.username == username, ScaleWeight.date == dt_str)
                            .first()
                        )
                        if not existing_w:
        ...
        if csv_files:
            ...
            with open(food_file, encoding="utf-8-sig") as f:
                ...
                for r in reader:
                    ...
                    meal = MealLog(
                        ...
                        canonical_name=r.get("Food Name", "Logged Food"),
                        serving_size=r.get("Serving Size"),
                        serving_qty=float(r.get("Serving Qty", 1.0) or 1.0),
                        serving_weight_g=float(r.get("Serving Weight (g)", 0) or 0)
                        if r.get("Serving Weight (g)")
                        else None,
                        calories=cals,
                        protein=p,
                        carbs=c,
