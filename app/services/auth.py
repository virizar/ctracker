...
def get_current_user(
    ...
    api_key_obj = db.query(APIKey).filter(APIKey.key == token, APIKey.is_active.is_(True)).first()
    if not api_key_obj:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid or revoked API Key")

    user = db.query(UserProfile).filter(UserProfile.username == api_key_obj.username).first()
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Associated user profile not found")
...
def verify_master_key(x_master_key: str = Security(master_key_header)):
    if not x_master_key or x_master_key != settings.MASTER_API_KEY:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Invalid or missing X-Master-Key header for admin actions"
        )
    return True
