...
from app.db.models import FoodCatalog, MealLog, ProcessedEvent, UserProfile
...
def get_or_create_food_catalog(
    ...
    catalog = (
        ...
        .filter(FoodCatalog.username == username, FoodCatalog.canonical_name.ilike(clean_name))
        .first()
    )
...
def create_meals_batch(
    payload: MealCreateRequest | list[MealCreateRequest] | BatchMealCreateRequest,
    ...
    if global_event_id:
        ...
        if existing:
            meals = (
                db.query(MealLog)
                .filter(MealLog.username == user.username, MealLog.client_event_id == global_event_id)
                .all()
            )
            return [
                MealResponse(
                    ...
                    food_catalog_id=m.food_catalog_id,
    ...
    for req in items_to_create:
        ...
        meal = MealLog(
            ...
            date=req.date,
            time=req.time or datetime.now(UTC).strftime("%I:%M %p"),
...
def search_food_catalog(
    ...
    results = (
        db.query(FoodCatalog)
        .filter(FoodCatalog.username == user.username, FoodCatalog.canonical_name.ilike(f"%{clean_q}%"))
        .order_by(FoodCatalog.usage_count.desc())
        .limit(20)
        .all()
    )
...
def get_meals(date: str, user: UserProfile = Depends(get_current_user), db: Session = Depends(get_db)):
...
def delete_meal(meal_id: str, user: UserProfile = Depends(get_current_user), db: Session = Depends(get_db)):
