...
from app.db.models import UserProfile, ScaleWeight, MealLog
...
async def import_file(
    ...
    try:
        ...
        for w_item in payload.weights:
            existing_w = db.query(ScaleWeight).filter(
                ScaleWeight.username == user.username,
                ScaleWeight.date == w_item.date
            ).first()
            if existing_w:
        ...
        for m_item in payload.meals:
            ...
            catalog_obj = get_or_create_food_catalog(
                db, user.username, c_name, m_item.serving_size, m_item.serving_weight_g,
                m_item.calories, m_item.protein, m_item.carbs, m_item.fat
            )
            meal = MealLog(
                username=user.username,
                date=m_item.date,
