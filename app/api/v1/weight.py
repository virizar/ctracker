...
from app.db.models import UserProfile, ScaleWeight, ProcessedEvent
...
def log_scale_weight(
    ...
    w_obj = db.query(ScaleWeight).filter(ScaleWeight.username == user.username, ScaleWeight.date == req.date).first()
    if not w_obj:
        w_obj = ScaleWeight(username=user.username, date=req.date, raw_weight=req.raw_weight, client_event_id=req.client_event_id)
        db.add(w_obj)
    else:
    ...
    return WeightLogResponse(
        id=w_obj.id,
        date=w_obj.date,
        raw_weight=w_obj.raw_weight,
        trend_weight=w_obj.trend_weight
    )
...
def get_weight_history(
    ...
    return [
        WeightLogResponse(id=r.id, date=r.date, raw_weight=r.raw_weight, trend_weight=r.trend_weight)
        for r in records
    ]

@router.delete("/{date}", status_code=status.HTTP_204_NO_CONTENT)
def delete_scale_weight(
    date: str,
    user: UserProfile = Depends(get_current_user),
    db: Session = Depends(get_db)
):
